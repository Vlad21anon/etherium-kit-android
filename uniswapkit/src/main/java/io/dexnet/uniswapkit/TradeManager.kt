...
import io.dexnet.uniswapkit.contract.*
import io.dexnet.uniswapkit.models.*
import io.dexnet.uniswapkit.models.Token.Erc20
import io.dexnet.uniswapkit.models.Token.Ether
...
class TradeManager(
    ...
    fun pair(tokenA: Token, tokenB: Token): Single<Pair> {
        ...
        return evmKit.call(pairAddress, io.dexnet.uniswapkit.contract.GetReservesMethod().encodedABI())
                .map { data ->
    ...
    private fun buildMethodForExactOut(tokenIn: Token, tokenOut: Token, path: List<Address>, to: Address, deadline: BigInteger, tradeData: TradeData, trade: Trade): ContractMethod {
        ...
        return when {
            tokenIn is Ether && tokenOut is Erc20 -> io.dexnet.uniswapkit.contract.SwapETHForExactTokensMethod(
                amountOut,
                path,
                to,
                deadline
            )
            tokenIn is Erc20 && tokenOut is Ether -> io.dexnet.uniswapkit.contract.SwapTokensForExactETHMethod(
                amountOut,
                amountInMax,
                path,
                to,
            ...
            tokenIn is Erc20 && tokenOut is Erc20 -> io.dexnet.uniswapkit.contract.SwapTokensForExactTokensMethod(
                amountOut,
                amountInMax,
                path,
                to,
    ...
    private fun buildMethodForExactIn(tokenIn: Token, tokenOut: Token, path: List<Address>, to: Address, deadline: BigInteger, tradeData: TradeData, trade: Trade): ContractMethod {
        ...
        return when {
            tokenIn is Ether && tokenOut is Erc20 -> io.dexnet.uniswapkit.contract.SwapExactETHForTokensMethod(
                amountOutMin,
                path,
                to,
                deadline
            )
            tokenIn is Erc20 && tokenOut is Ether -> io.dexnet.uniswapkit.contract.SwapExactTokensForETHMethod(
                amountIn,
                amountOutMin,
                path,
                to,
            ...
            tokenIn is Erc20 && tokenOut is Erc20 -> io.dexnet.uniswapkit.contract.SwapExactTokensForTokensMethod(
                amountIn,
                amountOutMin,
                path,
                to,
